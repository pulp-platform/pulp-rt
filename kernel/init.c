...
void __rt_wait_dispatch() {
  ...
  if(rt_core_id()) {
    ...
    pulp_write32(ARCHI_EU_DEMUX_ADDR+EU_BARRIER_DEMUX_OFFSET+EU_BARRIER_AREA_OFFSET_GET(0)+EU_HW_BARR_TRIGGER_SELF,1);
  } else {
    // Enable barrier wake-up event
    pulp_write32(ARCHI_EU_DEMUX_ADDR+EU_CORE_DEMUX_OFFSET+EU_CORE_MASK_OR, 1 << PULP_HW_BAR_EVENT);
    // Trigger barrier blocking and then trigger the FC
    pulp_read32(ARCHI_EU_DEMUX_ADDR+EU_BARRIER_DEMUX_OFFSET+EU_BARRIER_AREA_OFFSET_GET(0)+EU_HW_BARR_TRIGGER_WAIT_CLEAR);
    // Trigger the FC
