...
void himaxRegWrite(rt_camera_t *cam, unsigned int addr, unsigned char value){
  ...
  rt_i2c_write(cam->i2c, addr, value, call_event);
  rt_event_wait(call_event);
}

unsigned char himaxRegRead(rt_camera_t *cam, unsigned int addr){
  rt_event_t *call_event = rt_event_get_blocking(NULL);
  rt_i2c_read(cam->i2c, addr, &valueReg, call_event);
  rt_event_wait(call_event);
  return valueReg;
}
...
#ifdef ACTIVATE_I2C
        rt_event_t *call_event = rt_event_get_blocking(NULL);
        rt_i2c_conf(cam->i2c, 0x48, 0x200, call_event);
        rt_event_wait(call_event);
#endif
...
#ifdef ACTIVATE_I2C
  //cam->i2c = rt_i2c_open("i2c0", &cam->i2c_conf, 0);
  unsigned char i2c_ch = 6;
  camera->i2c = __rt_i2c_open_channel(i2c_ch, &camera->i2c_conf, NULL);
  if (camera->i2c == NULL) printf ("Filed to open I2C\n");
  // the I2C of Himax freq: 400kHz max.
  rt_event_t *call_event = rt_event_get_blocking(NULL);
  rt_i2c_conf(camera->i2c, 0x48, 0x100, call_event);
  rt_event_wait(call_event);
#endif
